...
- **test:** 23 unit tests (`src/fs/`), 8 HTTP-level tests (`tests/integration_fstemplates.rs`), and `ci-fstemplate-verify.sh` — template → seal → clone ×2 → iSCSI export → real open-iscsi initiator → `blkid` → `e2fsck -fn` → mount rw → write → umount → `e2fsck` again, checking that the two clones carry distinct UUIDs and that neither mounts read-only.
