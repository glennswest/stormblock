...
- **Known limitation (addressed the following day):** a clone of one of these templates **mounted on RouterOS but rejected every write** (#39). RouterOS's own `format-drive ext4` produces a filesystem carrying `HAS_JOURNAL`, `64BIT`, `FLEX_BG` and `METADATA_CSUM`, none of which this formatter emitted except `64bit` on request. That profile is now the default — see the 2026-08-12 entries — though the RouterOS write path itself is still unconfirmed.
