...
- **Known limitation:** a clone of one of these templates **mounts on RouterOS but rejects every write** (#39). RouterOS's own `format-drive ext4` produces a filesystem carrying `HAS_JOURNAL`, `64BIT`, `FLEX_BG` and `METADATA_CSUM`, none of which this formatter emits except `64bit` on request; matching that on-disk profile is open work, and the Linux kernel path (`e2fsck -fn`, mount, write) is what is verified today.
